...
public class ProxyHeadersTest
{
    ...
    public void testProxyRemovesConnectionHeadersFromServer () throws Exception
    {
        ...
        mockServer.when (request ().withMethod ("GET").withPath ("/connectionheaders"), Times.exactly (1)).respond (response ().withStatusCode (200).withBody ("success").withHeader ("Connection", "Transfer-Encoding, Dummy-Header").withHeader ("Transfer-Encoding", "identity").withHeader ("Dummy-Header", "dummy-value").withConnectionOptions (new ConnectionOptions ().withSuppressConnectionHeader (true)));
        ...
        // make sure we find the "chunked" header, which should not be removed
        boolean foundChunked = false;
        for (Header transferEncodingHeader : transferEncodingHeaders)
        {
            if ("chunked".equals (transferEncodingHeader.getValue ()))
            {
                foundChunked = true;
                break;
            }
        ...
        assertTrue ("Expected to find Transfer-Encoding: identity header value specified in response", foundChunked);
    }
}
