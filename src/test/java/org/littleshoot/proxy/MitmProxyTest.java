...
public class MitmProxyTest extends BaseProxyTest
{
                ...
                public HttpFilters filterRequest (HttpRequest originalRequest)
                {
                    return new HttpFiltersAdapter (originalRequest)
                    {
                        ...
                        public HttpObject serverToProxyResponse (HttpObject httpObject)
                        {
                            ...
                            {
                                responsePreBody.append (((HttpContent) httpObject).content ().toString (StandardCharsets.UTF_8));
                            }
                            return httpObject;
                        }
                        ...
                        public HttpObject proxyToClientResponse (HttpObject httpObject)
                        {
                            ...
                            {
                                responsePostBody.append (((HttpContent) httpObject).content ().toString (StandardCharsets.UTF_8));
                            }
                            return httpObject;
                        }
                    };
    ...
    {
        assertThat (method + " should have been seen in clientToProxyRequest filter", requestPreMethodsSeen, hasItem (method));
        assertThat (method + " should have been seen in proxyToServerRequest filter", requestPostMethodsSeen, hasItem (method));
    }

    private void assertMethodSeenInResponseFilters (HttpMethod method)
    {
        assertThat (method + " should have been seen as the original requests's method in serverToProxyResponse filter", responsePreOriginalRequestMethodsSeen, hasItem (method));
        assertThat (method + " should have been seen as the original requests's method in proxyToClientResponse filter", responsePostOriginalRequestMethodsSeen, hasItem (method));
    }
    ...
    private void assertResponseFromFiltersMatchesActualResponse ()
    {
        ...
        assertEquals ("Data received through HttpFilters.proxyToClientResponse should match response", lastResponse, responsePostBody.toString ());
    }
